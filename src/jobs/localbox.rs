...
pub fn run_job(spec: &LocalJobSpec) -> Result<PathBuf> {
    ...
    let launcher = format!(
        concat!(
            ...
            "    & powershell.exe -NoLogo -NoProfile -NonInteractive -ExecutionPolicy Bypass -File $payload *>&1 | ForEach-Object {{\r\n",
            ...
            "    $exitCode = 1\r\n",
            "}}\r\n",
            "[System.IO.File]::WriteAllText($exitFile, [string]$exitCode, $encoding)\r\n",
            "exit $exitCode\r\n",
        ),
        exports = exports,
    );
